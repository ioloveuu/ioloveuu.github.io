...
function _run(){													//snake运行
	time = setInterval(function(){
		...
		if(t.rows[snake[0].y].cells[snake[0].x].children[0]){
			if(t.rows[snake[0].y].cells[snake[0].x].children[0].id == 'fruit'){
...
function _continue(){				//继续游戏
	play.parentNode.style.display = 'none';
	_run();
}
window.onkeydown = function(e){
	if(_play.onoff){				//为空格控制赋值key
		if(e.keyCode == 32){
			difficult.blur();
			if(play.parentNode.style.display == 'none'){
				clearInterval(time);
				play.innerHTML = '继续游戏';
				play.href = 'javascript:_continue();';
				play.parentNode.style.display = 'block';
			}else{
				_continue();
			}
		}
	}
	if(play.parentNode.style.display == 'none'&&e.keyCode>=37&&e.keyCode<=40){   //严禁重按及反向
		if(e.keyCode != runWay&&e.keyCode != runWay+2&&e.keyCode != runWay-2){
			key = e.keyCode;
		}	
	}	
}
function _can_create(){							//随机生成一个可以创建fruit的td的下标值
...
function _create_fruit(){						//生成fruit
	var img = document.createElement('img');
	img.src = 'img/fruit'+Math.ceil(Math.random()*7)+'.png';
	img.id = 'fruit';
